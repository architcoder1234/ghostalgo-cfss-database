...
async function uploadImage(file, carId) {
  ...
  const { error } = await supabase.storage.from("car-images").upload(path, file, { contentType: file.type || "image/jpeg", upsert: false, cacheControl: "31536000" });
...
async function deleteImage(url) { const path = storagePathFromUrl(url); if (!path) return; const { error } = await supabase.storage.from("car-images").remove([path]); if (error) console.warn("Image delete warning", error); }
...
$("login-form").addEventListener("submit", async e => {
  ...
  if (data.user.id !== ADMIN_UID) { await supabase.auth.signOut(); return loginMsg("This account is not authorized as GhostAlgo admin.", "error"); }
  showAdmin(data.user);
});
...
async function checkSession() {
  ...
  if (data.session?.user) {
    if (data.session.user.id === ADMIN_UID) showAdmin(data.session.user); else await supabase.auth.signOut();
  }
}
supabase?.auth.onAuthStateChange((event, session) => { if (event === "SIGNED_OUT") { $("login-panel").hidden = false; $("admin-panel").hidden = true; resetForm(); } });
...
$("car-form").addEventListener("submit", async e => {
  ...
  try {
    ...
    if (editing) {
      const { error } = await supabase.from("cars").update(payload).eq("id", carId); if (error) throw error;
    } else {
      const { data, error } = await supabase.from("cars").insert(payload).select().single(); if (error) throw error; carId = data.id;
    }
    const file = $("car-image").files[0];
    if (file) {
      const url = await uploadImage(file, carId);
      const { error } = await supabase.from("cars").update({ image_url: url }).eq("id", carId); if (error) throw error;
...
$("delete-car").addEventListener("click", async () => {
  ...
  try {
    ...
    const { error } = await supabase.from("cars").delete().eq("id", editing.id); if (error) throw error;
...
$("import-cars").addEventListener("click", async () => {
  ...
  try {
    ...
    const { data: existing, error } = await supabase.from("cars").select("name,category"); if (error) throw error;
    ...
    for (let i = 0; i < missing.length; i += 100) { const { error } = await supabase.from("cars").insert(missing.slice(i, i + 100)); if (error) throw error; }
